...
class LDS(RandomVariable, Distribution):
    """
    ...
    """
    def __init__(self, GenerativeParams, xDim, yDim, y_data,
                 name='LDS', value=None, dtype=tf.float32,
                 reparameterization_type=FULLY_REPARAMETERIZED,
                 validate_args=True, allow_nan_stats=True):
        ...
        if 'RChol' in GenerativeParams:
            self.RChol = tf.Variable(initial_value=np.ndarray.flatten(
                GenerativeParams['RChol']), dtype=tf.float32, name='RChol')
            # cholesky of observation noise cov matrix
        else:
            self.RChol = tf.Variable(initial_value=np.random.randn(yDim) / 10,
                                     dtype=tf.float32, name='RChol')
            # cholesky of observation noise cov matrix
        ...
        if 'NN_XtoY_Params' in GenerativeParams:
            ...
            # output
            gen_nn = layers.Input(shape=(xDim,),
                                  name='%s_gen_nn_input' % name)
            gen_nn_d = layers.Dense(
                yDim, activation='linear',
                kernel_initializer=tf.orthogonal_initializer(),
                name='%s_gen_nn_dense' % name)(gen_nn)
    ...
    def sampleY(self):
        ...
        return (self.rate + tf.matmul(
            tf.random_normal((tf.shape(self.Xsamp)[0], self.yDim)),
    ...
    def _log_prob(self, value):
        ...
        LogDensity = -0.5 * tf.reduce_sum(
            tf.matmul(tf.transpose(resY), resY) * tf.diag(self.Rinv))

        LogDensity -= 0.5 * tf.reduce_sum(
            tf.matmul(tf.transpose(resX), resX) * self.Lambda)

        LogDensity -= 0.5 * tf.matmul(
            tf.matmul(tf.reshape(self.resX0, [1, -1]), self.Lambda0),
