...
def prep_injection_conditions(data, modes):
    ...
    for i in range(len(data)):
        ...
        curr_mode = modes[i].reshape((1, -1)).astype(tf.float32)
...
def prep_GAN_data(model_in, data, extra_cond=None):
    ...
    for i in range(len(data)):
        trial = data[i]
        g_samp = model.mrec.getSample()
        trial_postJ = tf.concat(model.mprior_goalie.draw_postJ(
            g_samp[:, model.mprior_goalie.yCols]),
                                model.mprior_ball.draw_postJ(
                                    g_samp[:, model.mprior_ball.yCols]),
            axis=1).eval()
