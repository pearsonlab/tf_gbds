...
def gen_data(n_trials, n_obs, sigma=np.log1p(np.exp(-10 * np.ones((1, 2)))),
    ...
    for _ in range(n_trials):
        ...
        g_b_x_mu = 0.5 * np.sin(2. * (np.linspace(0, 2 * np.pi, n_obs) -
                                       init_b_x))
        init_b_y = np.pi * (np.random.rand() * 2 - 1)
        g_b_y_mu = 0.5 * np.sin(2. * (np.linspace(0, 2 * np.pi, n_obs) -
                                       init_b_y))
        g_b_mu = np.hstack([g_b_x_mu.reshape(n_obs, 1),
                            g_b_y_mu.reshape(n_obs, 1)])
        g_b_lambda = np.array([16, 16], np.float32)
        g_b[0] = [0.5 * (np.random.rand() * 2 - 1),
                  0.5 * (np.random.rand() * 2 - 1)]
