...
class PKBiasLayer(tf.contrib.keras.layers.Layer):
    ...
    def __init__(self, incoming, srng, params,
                 param_init=tf.random_normal_initializer(mean=0, stddev=0.01),
                 num_biases=4, **kwargs):
    ...
    def get_ELBO(self, nbatches):
        ...
        """
        ELBO = (-tf.abs_(self.biases) / self.k -
                tf.reduce_sum(tf.log(2 * self.k)))
    ...
    def get_output_for(self, input, **kwargs):
        if self.draw_on_every_output:
            self.draw_biases()
        act_biases = tf.matmul(tf.reshape(self.mode.astype(tf.float32),
                               [1, -1]), self.biases)
