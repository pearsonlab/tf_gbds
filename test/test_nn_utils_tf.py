import numpy as np
import numpy.testing as npt
import tensorflow as tf
from keras import layers, models
from nn_utils_tf import get_network
from layers import PKBiasLayer, PKRowBiasLayer


batch_size = 256
input_dim = 64
output_dim = 8
(hidden_dim, num_layers) = (32, 4)
filt_size = 16
data = tf.random_normal((1, batch_size, input_dim), dtype=tf.float32)

def test_nn_utils():
	NN = get_network(batch_size, input_dim, output_dim, hidden_dim, num_layers, 
					 batchnorm=True, filt_size=filt_size)
	assert isinstance(NN, models.Model)

	pad_shape = tf.TensorShape.as_list(NN.get_layer("Padding").output.shape)
	assert pad_shape == [None, batch_size + filt_size - 1, input_dim]

	conv_shape = tf.TensorShape.as_list(NN.get_layer("Conv").output.shape)
	assert conv_shape == [None, batch_size, hidden_dim]

	output = NN(data)
	output_shape = tf.TensorShape.as_list(output.shape)
	assert output_shape == [1, batch_size, output_dim]
