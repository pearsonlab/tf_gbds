...
def test_DLGMLayer():
    ...
    u_nn_d = (tf.contrib.keras.layers.Dense(xDim*xDim, activation="linear",
              kernel_initializer=tf.orthogonal_initializer())(u_nn))
    u_net = tf.contrib.keras.models.Model(inputs=u_nn, outputs=u_nn_d)

    unc_d_nn = tf.contrib.keras.layers.Input((None, yDim))
    unc_d_nn_d = (tf.contrib.keras.layers.Dense(xDim*xDim, activation="linear",
